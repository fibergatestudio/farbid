        ...
        <div class="uk-card uk-card-default uk-card-small uk-border-rounded">
            @can('create_pages')
                <div class="uk-card-header uk-text-right">
                    {!! _l(trans('forms.button_add'), 'oleus.pages.create', ['a' => ['class' => 'uk-button uk-button-secondary uk-waves uk-border-rounded']]) !!}
                </div>
            @endcan
                        ...
                        <tbody>
                            @foreach($items as $item)
                                <tr>
                                    ...
                                    <td>
                                        {!! _l($item->title, 'oleus.pages.edit', ['p' => ['id' => $item->id]]) !!}
                                    </td>
                                    <td>{{ $item->_types($item->type) }}</td>
                                    @if($percentage_of_relation->count)
                                    ...
                                    @can('update_pages')
                                        <td class="uk-text-center">
                                            {!! _l('', 'oleus.pages.edit', ['p' => ['id' => $item->id], 'a' => ['class' => 'uk-text-primary',  'uk-icon' => 'icon: ui_mode_edit', 'uk-tooltip' => 'title: '. trans('forms.button_edit')]]) !!}
                                        </td>
                                    @endcan
                                </tr>
                            @endforeach
